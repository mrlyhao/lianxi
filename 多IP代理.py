...
def getProxies():
    init_proxies=[]
    for i in range(1,11):
        user_agent = GetUserAgent()#获取随机请求头
        headers = {"User-Agent":user_agent}
        url = 'http://www.xicidaili.com/nn/'+str(i)
        data = requests.get(url,headers=headers).text#解析前十页
...
def testProxy(currip):#判断爬取的代理是否有用
    tmp_proxies = []
...
def mulTestProxies(init_proxies):#多线程验证IP有效性
...
if __name__ =='__main__':#保存有效IP到本地
    init_proxies = getProxies()
    tmp_proxies = mulTestProxies(init_proxies)
    proxy_addrs = []
    for tmp_proxie in tmp_proxies:
        if len(tmp_proxie)!= 0:
            proxy_addrs.append(tmp_proxie)
    print(proxy_addrs)
